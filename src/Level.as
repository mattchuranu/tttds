package  
{
	...
	{
		[Embed(source = '/snd/warp.mp3')] private const WARP:Class;
		private var warp:Sfx = new Sfx(WARP);
		...
		public var playerPosArray:Array = [];
		public var anotherNumber:int = 0;
		...
		//public var ai:Vector.<PlayerAI> = new Vector.<PlayerAI>();
		...
		/*public function removeAI(index:int):void
		{
			ai.splice(index, 1);
    
			for (var i:uint = index; i < ai.length; i++) 
			{
				ai[i].position--;
			}
		}*/
		
		private function playerPos():void
		{
			var point:Point = new Point(Player.xx, Player.yy);
			
			playerPosArray[anotherNumber] = point;
